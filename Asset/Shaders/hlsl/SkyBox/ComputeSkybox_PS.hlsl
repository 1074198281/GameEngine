//#include "BaseScattering.hlsl"

#include "../Algorithm/Atmosphere/BaseScattering.hlsl"
...
float3 ComputeSingleScattering(float3 startPos, float3 rayDir, float rayLength)
{
    ...
    for (int i = 0; i < SAMPLE_COUNT; ++i)
    {
        ...
        float rayleighPhase = RayleighPhase(cosTheta); //3.0 / (16.0 * PI) * (1.0 + cosTheta * cosTheta);
