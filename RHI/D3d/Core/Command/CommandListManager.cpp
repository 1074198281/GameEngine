...
D3dGraphicsCore::CommandQueue::~CommandQueue()
{
    Shutdown();
}

void D3dGraphicsCore::CommandQueue::Shutdown()
{
...
D3dGraphicsCore::CommandListManager::CommandListManager() :
    m_Device(nullptr),
...
void D3dGraphicsCore::CommandListManager::Shutdown()
{
...
void D3dGraphicsCore::CommandQueue::Create(ID3D12Device* pDevice)
{
...
void D3dGraphicsCore::CommandListManager::Create(ID3D12Device* pDevice)
{
    ASSERT(pDevice != nullptr);
...
void D3dGraphicsCore::CommandListManager::CreateNewCommandList(D3D12_COMMAND_LIST_TYPE Type, ID3D12GraphicsCommandList** List, ID3D12CommandAllocator** Allocator)
{
...
uint64_t D3dGraphicsCore::CommandQueue::ExecuteCommandList(ID3D12CommandList* List)
{
...
uint64_t D3dGraphicsCore::CommandQueue::IncrementFence(void)
{
    std::lock_guard<std::mutex> LockGuard(m_FenceMutex);
    m_CommandQueue->Signal(m_pFence, m_NextFenceValue);
    return m_NextFenceValue++;
...
bool D3dGraphicsCore::CommandQueue::IsFenceComplete(uint64_t FenceValue)
{
...
void D3dGraphicsCore::CommandQueue::StallForFence(uint64_t FenceValue)
{
    CommandQueue& Producer = D3dGraphicsCore::g_CommandManager.GetQueue((D3D12_COMMAND_LIST_TYPE)(FenceValue >> 56));
    m_CommandQueue->Wait(Producer.m_pFence, FenceValue);
}

void D3dGraphicsCore::CommandQueue::StallForProducer(CommandQueue& Producer)
{
    ASSERT(Producer.m_NextFenceValue > 0);
    m_CommandQueue->Wait(Producer.m_pFence, Producer.m_NextFenceValue - 1);
}

void D3dGraphicsCore::CommandQueue::WaitForFence(uint64_t FenceValue)
{
...
void D3dGraphicsCore::CommandListManager::WaitForFence(uint64_t FenceValue)
{
    CommandQueue& Producer = D3dGraphicsCore::g_CommandManager.GetQueue((D3D12_COMMAND_LIST_TYPE)(FenceValue >> 56));
    Producer.WaitForFence(FenceValue);
}

ID3D12CommandAllocator* D3dGraphicsCore::CommandQueue::RequestAllocator()
{
...
void D3dGraphicsCore::CommandQueue::DiscardAllocator(uint64_t FenceValue, ID3D12CommandAllocator* Allocator)
{
