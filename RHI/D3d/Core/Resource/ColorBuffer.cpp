...
void D3dGraphicsCore::ColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize, uint32_t NumMips)
{
    ...
    {
        m_RTVHandle = D3dGraphicsCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        m_SRVHandle = D3dGraphicsCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
    ...
    for (uint32_t i = 0; i < NumMips; ++i)
    {
        if (m_UAVHandle[i].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN)
            m_UAVHandle[i] = D3dGraphicsCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
...
void D3dGraphicsCore::ColorBuffer::CreateFromSwapChain(const std::wstring& Name, ID3D12Resource* BaseResource)
{
    AssociateWithResource(D3dGraphicsCore::g_Device, Name, BaseResource, D3D12_RESOURCE_STATE_PRESENT);
    ...
    m_RTVHandle = D3dGraphicsCore::AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    D3dGraphicsCore::g_Device->CreateRenderTargetView(m_pResource.Get(), nullptr, m_RTVHandle);
}

void D3dGraphicsCore::ColorBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t NumMips,
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
{
    ...
    CreateTextureResource(D3dGraphicsCore::g_Device, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(D3dGraphicsCore::g_Device, Format, 1, NumMips);
}

void D3dGraphicsCore::ColorBuffer::CreateArray(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t ArrayCount,
    DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMem)
{
    D3D12_RESOURCE_FLAGS Flags = CombineResourceFlags();
    ...
    CreateTextureResource(D3dGraphicsCore::g_Device, Name, ResourceDesc, ClearValue, VidMem);
    CreateDerivedViews(D3dGraphicsCore::g_Device, Format, ArrayCount, 1);
}

void D3dGraphicsCore::ColorBuffer::GenerateMipMaps(CommandContext& BaseContext)
{
    if (m_NumMipMaps == 0)
        return;
    ...
    Context.SetRootSignature(D3dGraphicsCore::g_CommonRS);
