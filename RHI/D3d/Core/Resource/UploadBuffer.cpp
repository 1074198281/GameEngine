...
void* D3dGraphicsCore::UploadBuffer::Map(void)
{
    void* Memory;
    D3D12_RANGE range = CD3DX12_RANGE(0, m_BufferSize);
    m_pResource->Map(0, &range, &Memory);
    return Memory;
}

void D3dGraphicsCore::UploadBuffer::Unmap(size_t begin, size_t end)
{
    D3D12_RANGE range = CD3DX12_RANGE(begin, std::min(end, m_BufferSize));
