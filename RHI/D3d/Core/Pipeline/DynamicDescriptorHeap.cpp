...
std::mutex D3dGraphicsCore::DynamicDescriptorHeap::sm_Mutex;
std::vector<Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>> D3dGraphicsCore::DynamicDescriptorHeap::sm_DescriptorHeapPool[2];
std::queue<std::pair<uint64_t, ID3D12DescriptorHeap*>> D3dGraphicsCore::DynamicDescriptorHeap::sm_RetiredDescriptorHeaps[2];
std::queue<ID3D12DescriptorHeap*> D3dGraphicsCore::DynamicDescriptorHeap::sm_AvailableDescriptorHeaps[2];

ID3D12DescriptorHeap* D3dGraphicsCore::DynamicDescriptorHeap::RequestDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
{
    std::lock_guard<std::mutex> LockGuard(sm_Mutex);

    uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;

    while (!sm_RetiredDescriptorHeaps[idx].empty() && D3dGraphicsCore::g_CommandManager.IsFenceComplete(sm_RetiredDescriptorHeaps[idx].front().first))
    {
    ...
    else
    {
        ...
        ASSERT_SUCCEEDED(D3dGraphicsCore::g_Device->CreateDescriptorHeap(&HeapDesc, IID_PPV_ARGS(&HeapPtr)));
        sm_DescriptorHeapPool[idx].emplace_back(HeapPtr);
        return HeapPtr.Get();
    }
}

void D3dGraphicsCore::DynamicDescriptorHeap::DiscardDescriptorHeaps(D3D12_DESCRIPTOR_HEAP_TYPE HeapType, uint64_t FenceValue, const std::vector<ID3D12DescriptorHeap*>& UsedHeaps)
{
    uint32_t idx = HeapType == D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER ? 1 : 0;
    std::lock_guard<std::mutex> LockGuard(sm_Mutex);
...
void D3dGraphicsCore::DynamicDescriptorHeap::RetireCurrentHeap(void)
{
...
void D3dGraphicsCore::DynamicDescriptorHeap::RetireUsedHeaps(uint64_t fenceValue)
{
    DiscardDescriptorHeaps(m_DescriptorType, fenceValue, m_RetiredHeaps);
    m_RetiredHeaps.clear();
}

D3dGraphicsCore::DynamicDescriptorHeap::DynamicDescriptorHeap(CommandContext& OwningContext, D3D12_DESCRIPTOR_HEAP_TYPE HeapType)
    : m_OwningContext(OwningContext), m_DescriptorType(HeapType)
{
    m_CurrentHeapPtr = nullptr;
    m_CurrentOffset = 0;
    m_DescriptorSize = D3dGraphicsCore::g_Device->GetDescriptorHandleIncrementSize(HeapType);
}

D3dGraphicsCore::DynamicDescriptorHeap::~DynamicDescriptorHeap()
{
}

void D3dGraphicsCore::DynamicDescriptorHeap::CleanupUsedHeaps(uint64_t fenceValue)
{
    RetireCurrentHeap();
...
inline ID3D12DescriptorHeap* D3dGraphicsCore::DynamicDescriptorHeap::GetHeapPointer()
{
...
uint32_t D3dGraphicsCore::DynamicDescriptorHeap::DescriptorHandleCache::ComputeStagedSize()
{
...
void D3dGraphicsCore::DynamicDescriptorHeap::DescriptorHandleCache::CopyAndBindStaleTables(
...
void D3dGraphicsCore::DynamicDescriptorHeap::CopyAndBindStagedTables(DescriptorHandleCache& HandleCache, ID3D12GraphicsCommandList* CmdList,
    void (STDMETHODCALLTYPE ID3D12GraphicsCommandList::* SetFunc)(UINT, D3D12_GPU_DESCRIPTOR_HANDLE))
{
...
void D3dGraphicsCore::DynamicDescriptorHeap::UnbindAllValid(void)
{
    m_GraphicsHandleCache.UnbindAllValid();
    m_ComputeHandleCache.UnbindAllValid();
}

D3D12_GPU_DESCRIPTOR_HANDLE D3dGraphicsCore::DynamicDescriptorHeap::UploadDirect(D3D12_CPU_DESCRIPTOR_HANDLE Handle)
{
    if (!HasSpace(1))
    {
        RetireCurrentHeap();
...
void D3dGraphicsCore::DynamicDescriptorHeap::DescriptorHandleCache::UnbindAllValid()
{
    m_StaleRootParamsBitMap = 0;
...
void D3dGraphicsCore::DynamicDescriptorHeap::DescriptorHandleCache::StageDescriptorHandles(UINT RootIndex, UINT Offset, UINT NumHandles, const D3D12_CPU_DESCRIPTOR_HANDLE Handles[])
{
...
void D3dGraphicsCore::DynamicDescriptorHeap::DescriptorHandleCache::ParseRootSignature(D3D12_DESCRIPTOR_HEAP_TYPE Type, const RootSignature& RootSig)
{
