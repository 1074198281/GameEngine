...
void D3dGraphicsCore::RootSignature::DestroyAll(void)
{
    s_RootSignatureHashMap.clear();
}

void D3dGraphicsCore::RootSignature::InitStaticSampler(
    UINT Register,
...
void D3dGraphicsCore::RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags)
{
    if (m_Finalized)
        return;
